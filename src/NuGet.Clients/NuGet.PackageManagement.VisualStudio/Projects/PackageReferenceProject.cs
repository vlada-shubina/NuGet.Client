    ...
    public abstract class PackageReferenceProject<T, U> : BuildIntegratedNuGetProject, IPackageReferenceProject where T : ICollection<U>, new()
    {
        ...
        public virtual async Task<ProjectPackages> GetInstalledAndTransitivePackagesAsync(CancellationToken token)
        {
            ...
            if (await ExperimentUtility.IsTransitiveOriginExpEnabled.GetValueAsync(token))
            {
                ...
                transitivePackagesWithOrigins = calculatedInstalledPackages
                    .Select(packageRef =>
                    {
        ...
        protected abstract IReadOnlyList<PackageReference> ResolvedInstalledPackagesList(IEnumerable<LibraryDependency> libraries, NuGetFramework targetFramework, IReadOnlyList<LockFileTarget> targets, T installedPackages);
        ...
        private protected IReadOnlyList<PackageReference> GetPackageReferences(
            ...
            IReadOnlyList<LockFileTarget> targets)
        {
            return libraries
                           ...
                           .Select(library => new BuildIntegratedPackageReference(library, targetFramework, GetPackageReferenceUtility.UpdateResolvedVersion(library, targetFramework, targets, installedPackages))).ToList();
        }
