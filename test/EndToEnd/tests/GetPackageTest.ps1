...
function Test-GetPackageForFSharpProjectReturnsCorrectPackages {
    ...
    # Act
    $result = @(Get-Package -ProjectName $p.Name[2]) | where { $_.id -ne "FSharp.Core" }
...
# Fsharp patch Tuesday dependencies takes time to propogate into all feeds. 
...
function GetFSharpProjectWithPreviewDependencies()
{
    ...
    $node.InsertAfter($packageIdNode, $node.LastChild)
