...
Invoke-BuildStep 'Running Restore' {
    # Restore
    $restoreArgs = "build\build.proj", "/restore:false", "/t:EnsurePackageReferenceVersionsInSolution", "/p:Configuration=$Configuration"
    if ($Binlog)
    {
    ...
    $restoreArgs = "build\build.proj", "/restore:false", "/t:RestoreVS", "/p:Configuration=$Configuration", "/p:ReleaseLabel=$ReleaseLabel", "/p:IncludeApex=$IncludeApex"
...
Invoke-BuildStep $VSMessage {
    ...
    $buildArgs = 'build\build.proj', "/restore:false", "/t:$VSTarget", "/p:Configuration=$Configuration", "/p:ReleaseLabel=$ReleaseLabel", "/p:IncludeApex=$IncludeApex"
...
Invoke-BuildStep 'Running Restore RTM' {
    ...
    # Restore for VS
    $restoreArgs = "build\build.proj", "/restore:false", "/t:RestoreVS", "/p:Configuration=$Configuration", "/p:BuildRTM=true", "/p:ReleaseLabel=$ReleaseLabel", "/p:ExcludeTestProjects=true"
...
Invoke-BuildStep 'Packing RTM' {
    ...
    # Build and (If not $SkipUnitTest) Pack, Core unit tests, and Unit tests for VS
    $packArgs = "build\build.proj", "/restore:false", "/t:BuildVS`;Pack", "/p:Configuration=$Configuration", "/p:BuildRTM=true", "/p:ReleaseLabel=$ReleaseLabel", "/p:ExcludeTestProjects=true"
